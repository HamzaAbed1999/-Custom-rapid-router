...
def level_editor(request, artifactId=None, levelId=None):
    ...
    **Template:**
    ...
    if artifactId is None:
        artifactId=Level.objects.filter(id=levelId).first().subtitle
...
def Artifact_level_editor(request, artifactid=None):
    """Renders the level editor page.
    ...
    **Template:**
    ...
    if artifact_id:
        # level = Level.objects.get(id=artifact_id)
        # user_profile = UserProfile.objects.get(id=level.owner_id)
        #
        # if request.user.id == user_profile.user_id:
        # context["level"] = artifact_id
        context["artifactId"]=artifact_id
    return render(request, "game/level_editor.html", context=context)
...
def load_level_for_editor(request, levelID):
    ...
    response = {"owned": 'me', "level": level_dict}
    ...
    if pattern.match(data["name"]):
        ...
        if levelId is None:
            ...
            artifact_data = {
                ...
                'level_id': level.id,  # Include the level ID
                'artifact_id':artifactid
            }
            ...
            if response.status_code == 200:
                ...
                print(f'Failed to update artifact on Server A. Status code: {response.status_code}')
