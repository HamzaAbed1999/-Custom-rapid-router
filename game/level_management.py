...
def save_level(level, data):
    ...
    level.subtitle = data.get("artifact_id")
    if data.get("lesson") != None:
        level.lesson = data.get("lesson")
    if data.get("hint") != None:
        level.hint = data.get("hint")

        # Retrieve or create an App instance to assign to Artifacts
    # app, _ = App.objects.get_or_create(
    #     name="Rapid Router"  # Replace with the actual app name or retrieve it dynamically as needed
    # )



    # Create or update the artifact with the app assignment
    # artifact, created = Artifacts.objects.update_or_create(
    #     name=level.name,
    #     author_id=94,
    #     defaults={"artifact_data": artifact_data, "app": app}  # Assign a valid app instance here
    # )
    level.save()
    # print(f"{'Created' if created else 'Updated'} artifact:", artifact)
    set_decor(level, data["decor"])
    set_blocks(level, data["blocks"])
    # Store the level data in the artifact
